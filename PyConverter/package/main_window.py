    ...
    def convert_images(self):
        for image_lw_item in self.images_to_convert:
            if self.runs and not image_lw_item.processed:
                ...
                self.image_converted.emit(image, success)
...
class MainWindow(QtWidgets.QWidget):
    ...
    def create_widgets(self):
        ...
        self.lbl_dropInfo = QtWidgets.QLabel("^ Glissez vos fichiers ici")
    ...
    def convert_images(self):
        ...
        self.prg_dialog = QtWidgets.QProgressDialog("Conversion en cours...", "Annuler", 1, len(images_a_convertir), self)
    ...
    def dropEvent(self, event):
        event.accept()
        for urk in event.mimeData().urls():
            self.add_file(path=urk.toLocalFile())
